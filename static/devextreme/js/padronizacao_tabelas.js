...
// default da para costumizar a tabela, tem que ter em todas as tabelas
...
function replaceHeader() {
    ...
    $('.dx-datagrid-search-panel, .dx-treelist-search-panel').addClass('form-control').off('click').on('click', function(){$(this).find('.form-control').focus();}).css('width', '').find('.dx-texteditor-input').css('border', '0').addClass('form-control').attr('placeholder', 'Pesquisar nesta tabela...').parent().find('.dx-icon-search').remove();
...
let showInfo = function(dataField, data, dataType, _this, title, itemExampleValue) {
    ...
    let popupOptions = {
        ...
        contentTemplate: function(contentElement) {
            ...
            if(dataType === 'string'){} else if (dataType === 'number'){
...
function filterRow(container, options, headerDefault){
    ...
    let button = $("<span/>")
        .click(function(e){
            ...
            $(".form-control.select2-single").select2({    templateResult: formatStat,"language": {"noResults": function(){return ``;}}, escapeMarkup: function (markup) {return markup;}, theme: 'bootstrap'});
            e.stopPropagation();
        })
        .html(``)
